...
import { isAuthHeader } from "../headers.js";
import { Authentication, AuthType } from "../types.js";
import { HTTP, parseHTTPAuthHeader } from "./authentication-http.js";
...
const parseAPIKeyAuthCookie = (cookieStr: string): Array<APIKeyAuthCookie> => {
  ...
  const filteredSecurityNames = Object.keys(cookies).filter((cookieKey) =>
    cookieSecurityNames.some((str) => cookieKey.toLowerCase().includes(str)),
  );
